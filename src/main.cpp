...
class entity{
	public:
		...
		std::vector<float> createVertices(float ratio, float res){
			std::vector<float> vertices;			
			for (float i=0; i<=res; i++){
				float  alpha = (i * pi) / res; 
				for (float j=0; j <=res; j++){
					float theta = (j*2.0f*pi) / res;
					float currentRadius = ratio * std::sin(alpha);
				
					
					vertices.push_back(std::cos(theta)* currentRadius); //x
					vertices.push_back(std::cos(alpha)* ratio); //y
					vertices.push_back(std::sin(theta)* currentRadius); //z
				}
		...
		std::vector<unsigned int> createIndices(int res) {
		...
		entity(glm::vec3 initPosition, glm::vec3 initVelocity, double mass, float ratio, glm::vec4 color){
			...
			std::vector<float> v = createVertices(ratio, 32);
			std::vector<unsigned int> i = createIndices(32);
			creatingVBOVAO(v, i);
		}

	void creatingVBOVAO(const std::vector<float> vertices, const std::vector<unsigned int> indices){
